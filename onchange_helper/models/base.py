...
class Base(models.AbstractModel):
    ...
    def play_onchanges(self, values, onchange_fields):
        ...
        ###########
        # stupid hack to fix the issue with sub-specs not including fields that are no longer on the comodel
        for fname, fspec in onchange_specs.items():
            if "fields" in fspec:
                field = self._fields.get(fname)
                if field and field.relational:
                    comodel = self.env[field.comodel_name]
                    fspec["fields"] = {
                        k: v for k, v in fspec["fields"].items() if k in comodel._fields
                    }
        ############
